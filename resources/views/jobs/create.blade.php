<x-layout>
  <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
    ...
    <form method="POST" action="{{ route('jobs.store') }}" enctype="multipart/form-data">
      ...
      <x-inputs.textarea id="benefits" name="benefits" label="Benefits"
        placeholder="Health insurance, 401k, paid time off" />

      <x-inputs.text id="tags" name="tags" label="Tags (comma-separated)" type="text"
        placeholder="development,coding,java,python" />
      {{-- attribute binding --}}
      {{-- Without the colon, like options="[...]", Blade would pass the literal string "['Full-Time' => 'Full-Time',
      ...]" to the component, not the actual PHP array. --}}
      ...
      <x-inputs.textarea id="company_description" name="company_description" label="Company Description"
